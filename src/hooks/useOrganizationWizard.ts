    ...
    async (userId: string): Promise<boolean> => {
      ...
      try {
        ...
        if (allDepartments.length > 0) {
          ...
          if (tipoEspError) {
            console.error(
              "Erro ao criar tipo de especialização:",
              tipoEspError
            );
            throw new Error(
              `Erro ao criar tipo de especialização: ${tipoEspError.message}`
            );
          }
          ...
          const especializacoes: Array<{
            nome: string;
            tipo_especializacao_id: string;
          }> = [];

          Object.entries(wizardData.departmentSpecializations).forEach(
            ([deptTipo, specs]) => {
              specs.forEach((spec) => {
                if (spec.trim()) {
                  // Garantir que não são vazias
          ...
          if (especializacoes.length > 0) {
            // Remover duplicatas
            const uniqueEspecializacoes = especializacoes.filter(
              (esp, index, self) =>
            ...
            if (specError) {
              ...
              throw new Error(
                `Erro ao criar especializações: ${specError.message}`
              );
            }
