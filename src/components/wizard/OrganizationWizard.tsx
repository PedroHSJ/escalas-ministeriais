...
import { CheckCircle, Circle, Building2, Users, Settings, Sparkles, X, Plus } from 'lucide-react';
...
export function OrganizationWizard({ userId, onComplete }: OrganizationWizardProps) {
  ...
  const [tempSpecializationInputs, setTempSpecializationInputs] = useState<Record<string, string>>({});
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 1:
        return wizardData.organizationName.trim().length >= 3 && wizardData.organizationType;
      case 2:
  ...
  const removeCustomDepartment = (index: number) => {
    ...
    updateWizardData({ customDepartments: updated });
  };
    ...
    switch (currentStep) {
      case 1:
        return (
          <div className="space-y-6">
            <div>
              <Label htmlFor="orgName">Nome da {organizationTemplates[wizardData.organizationType]?.terminologia.organizacao || 'Organização'}</Label>
              <Input
                id="orgName"
                ...
                {Object.entries(organizationTemplates).map(([key, template]) => (
                  <Card
                    ...
                    className={`cursor-pointer transition-all hover:shadow-md ${
                      wizardData.organizationType === key ? 'ring-2 ring-primary' : ''
                    }`}
                    onClick={() => setOrganizationType(key)}
                  >
                    <CardContent className="p-4 text-center">
                      <div className="mb-2">
                        {key === 'igreja' && <Building2 className="mx-auto h-8 w-8" />}
                        {key === 'empresa' && <Users className="mx-auto h-8 w-8" />}
                        {key === 'grupo' && <Sparkles className="mx-auto h-8 w-8" />}
                      </div>
                      <h3 className="font-medium capitalize">{template.terminologia.organizacao}</h3>
                      <p className="text-sm text-muted-foreground mt-1">
                        {key === 'igreja' && 'Igrejas, congregações e comunidades religiosas'}
                        {key === 'empresa' && 'Empresas, startups e corporações'}
                        {key === 'grupo' && 'ONGs, grupos comunitários e associações'}
                      </p>
                    </CardContent>
                  </Card>
                ))}
          ...
          <div className="space-y-6">
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                  {wizardData.template.departamentos.map((dept) => (
                    <div key={dept.tipo} className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor={dept.tipo} className="flex-1">{dept.nome}</Label>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="border-t pt-4">
                <Label htmlFor="customDept">Adicionar {wizardData.template?.terminologia.departamento || 'Departamento'} Personalizado</Label>
              ...
              {wizardData.customDepartments.length > 0 && (
                <div className="mt-4">
                  <Label>Departamentos Personalizados</Label>
      ...
      case 3:
        return (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-medium mb-4">
                Configurar {wizardData.template?.terminologia.especializacao || 'Especializações'}
              </h3>
              <p className="text-muted-foreground mb-6">
                Você pode personalizar as especializações para cada departamento. 
                Digite separado por vírgulas ou adicione uma por vez.
              </p>

              {[...wizardData.selectedDepartments, ...wizardData.customDepartments].map((dept) => (
                <Card key={dept.tipo} className="p-4">
                  <h4 className="font-medium mb-3">{dept.nome}</h4>
                  
                  {/* Mostrar especializações atuais */}
                  {(wizardData.departmentSpecializations[dept.tipo] || []).length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-3">
                      {(wizardData.departmentSpecializations[dept.tipo] || []).map((spec, index) => (
                        <Badge key={index} variant="outline" className="flex items-center gap-1">
                          {spec}
                          <button
                            onClick={() => removeSpecialization(dept.tipo, index)}
                            className="ml-1 text-red-500 hover:text-red-700"
                          >
                            <X className="h-3 w-3" />
                          </button>
                  ...
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      <Input
                        ...
                        onChange={(e) => handleSpecializationInput(dept.tipo, e.target.value)}
                        onKeyPress={(e) => {
                          if (e.key === 'Enter') {
                            e.preventDefault();
                    ...
                    <p className="text-xs text-muted-foreground">
                      Exemplo: Violão, Guitarra, Bateria ou pressione Enter para adicionar
                    </p>
                  </div>
                  ...
                  {wizardData.template?.especializacoes[dept.tipo] && (
                    <div className="mt-3 pt-3 border-t">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {wizardData.template.especializacoes[dept.tipo]
                          .filter(spec => !(wizardData.departmentSpecializations[dept.tipo] || []).includes(spec))
                          .map((spec) => (
                          <Button
                            key={spec}
                            variant="ghost"
                            ...
                            onClick={() => addSingleSpecialization(dept.tipo, spec)}
                          >
                            + {spec}
                          </Button>
                        ))}
      ...
      case 4:
        return (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-medium mb-4">Resumo da Configuração</h3>
              
              <Card className="p-4 mb-4">
                <h4 className="font-medium mb-2">Organização</h4>
                <p><strong>Nome:</strong> {wizardData.organizationName}</p>
                <p><strong>Tipo:</strong> {wizardData.template?.terminologia.organizacao}</p>
              </Card>

              <Card className="p-4 mb-4">
                <h4 className="font-medium mb-2">{wizardData.template?.terminologia.departamento}s</h4>
                <div className="space-y-3">
                  {[...wizardData.selectedDepartments, ...wizardData.customDepartments].map((dept, index) => (
                    <div key={index} className="border-l-2 border-primary pl-3">
                      <div className="flex justify-between items-start mb-1">
                        <span className="font-medium">{dept.nome}</span>
                        <Badge variant="outline">
                          {(wizardData.departmentSpecializations[dept.tipo] || []).length} especializações
                        </Badge>
                      </div>
                      {(wizardData.departmentSpecializations[dept.tipo] || []).length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {(wizardData.departmentSpecializations[dept.tipo] || []).map((spec, specIndex) => (
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                <p className="text-sm text-blue-800">
                  ✨ Sua organização será criada com {[...wizardData.selectedDepartments, ...wizardData.customDepartments].length} departamentos e {""}
                  {Object.values(wizardData.departmentSpecializations).flat().length} especializações configuradas.
                </p>
              </div>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {currentStep === 2 && 'Selecione os departamentos que sua organização possui'}
            {currentStep === 3 && 'Configure as especializações para cada departamento'}
